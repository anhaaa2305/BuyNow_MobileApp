...
import com.haluuvananh.ecommerceappv2.model.CategoryModel;
...
public class AddCategoryActivity extends AppCompatActivity {
                ...
                .addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                    ...
                    public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                        if (task.isSuccessful()) {
                            categoryId = Integer.parseInt(Objects.requireNonNull(task.getResult().get("lastCategoryId")).toString()) + 1;
    ...
    private void addToFirebase() {
        ...
        categoryId = Integer.parseInt(Objects.requireNonNull(idEditText.getText()).toString());
        String name = Objects.requireNonNull(nameEditText.getText()).toString();
        String desc = Objects.requireNonNull(descEditText.getText()).toString();
        String color = Objects.requireNonNull(colorEditText.getText()).toString();

        CategoryModel category = new CategoryModel(name, categoryImage, color, desc, categoryId);

        FirebaseUtil.getCategories().add(category)
                .addOnSuccessListener(documentReference -> FirebaseUtil.getDetails().update("lastCategoryId", categoryId)
    ...
    private void removeImage() {
        ...
        alertDialog
                ...
                .setConfirmClickListener(sweetAlertDialog -> {
                    ...
                    FirebaseUtil.getCategoryImageReference(String.valueOf(categoryId)).delete();
                    alertDialog.dismiss();
                }).show();
    }
    ...
    private boolean validate() {
        ...
        if (Objects.requireNonNull(nameEditText.getText()).toString().trim().length() == 0) {
        ...
        if (Objects.requireNonNull(colorEditText.getText()).toString().trim().length() == 0) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == 101) {
            if (data != null && data.getData() != null) {
                ...
                FirebaseUtil.getCategoryImageReference(String.valueOf(categoryId)).putFile(imageUri)
                        .addOnCompleteListener(t -> {
                            imageUploaded = true;

                            FirebaseUtil.getCategoryImageReference(String.valueOf(categoryId)).getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                                @Override
